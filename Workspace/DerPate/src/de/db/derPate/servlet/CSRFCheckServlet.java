...
public abstract class CSRFCheckServlet extends HttpServlet {
	 ...
	 */
	protected boolean checkAndInvalidateCSRFToken(@NonNull HttpServletRequest request) {
		String csrfToken = this.getCSRFToken(request);
		HttpSession session = request.getSession();

		return (csrfToken != null && session != null
				&& CSRFPrevention.checkAndInvalidateToken(session, this.userform, csrfToken));
	}
	...
	protected boolean handleCSRFToken(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response) {
		boolean hasToken = this.checkAndInvalidateCSRFToken(request);
		if (!hasToken) {
