...
public class CSRFPreventionUtil {
	 ...
	 * attributes
	 */
	public static final String SESSION_ATTRIBUTE_PREFIX = "csrfTokenMap_";
	...
	@NonNull
	private static LinkedHashMap<String, Long> getFormTokens(@NonNull HttpSession session, @NonNull String form) {
		...
		if (sessionTokens != null && sessionTokens instanceof LinkedHashMap) {
			formTokens = (LinkedHashMap<String, Long>) sessionTokens;
		}
	...
	public static boolean checkToken(@NonNull HttpSession session, @NonNull String formidentifier,
		...
		return (formTokens.containsKey(token)
				&& formTokens.get(token) > System.currentTimeMillis() - (timeoutInSeconds * 1000));
	}
	 ...
	 */
	public static boolean checkAndInvalidateToken(@NonNull HttpSession session, @NonNull String formidentifier,
			@NonNull String token, int timeoutInSeconds) {
